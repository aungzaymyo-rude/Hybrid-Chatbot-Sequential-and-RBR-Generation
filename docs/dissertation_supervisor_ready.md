# Dissertation: Design and Development of a Hybrid Chatbot for Medical Haematology using Sequential Models for Intent Classification and Retrieval-Based Response Generation
...
At the time of final documentation, the master dataset contained 5,391 labelled utterances across 29 intents. The general model dataset contained 2,670 utterances across 21 intents, while the report model dataset contained 4,777 utterances across 26 intents. Large classes included `cbc_info=312`, `coag_test=300`, `sample_collection=214`, and `report_numeric_result_analysis=1,219`. Additional data was added iteratively to improve weak classes such as `small_talk`, `capability_query`, `coag_test`, and multi-value report-analysis prompts.
...
### 3.8 Safety Rules, Communication Intents, Model Switching, and Report Analysis
...
- Report-analysis extraction and interpretation.
- HTTPS deployment helpers and certificate monitoring.
...
[Insert *Figure 4.1. Automated test execution summary showing 52 passing tests* here.]
...
The final browser UI also exposed a model-aware coverage panel so that the supported haematology topics of the `general` and `report` models were visible to the user. This reduced ambiguity around what each model was expected to do and aligned the interface more closely with the underlying architecture.
